...
1. Place the jar file and the following files in the same folder as Stocks.jar:
    ...
    - users folder (already has details regarding 1 users with file - karthikjb10File.txt)
...
<user>
    ...
    <portfolio id="Emergency Funds">         //Name of the portfolio is unique and is the id
        <stock id="IBM">        //Name of the stock is unique within the Portfolio and is the id
            ...
            <symbol>IBM</symbol>     //Ticker symbol of the stock purchased
        </stock>
    </portfolio>
    <fportfolio id="College">
        <stock id="Microsoft">
            <cost>119.97</cost>
            <quantity>20.0</quantity>
            <time>2019/04/03</time>
            <symbol>MSFT</symbol>
            <commission>10.0</commission> //Field added for commission fees to be input by the user.
        </stock>
    </fportfolio>
</user>

To include a plan while loading user data follow the following template and add it to the same file
as the portfolios, just below the portfolio tag, inside the user tag.

<plans>
    <plan>
        <name>Plan7</name>
        <stime>2022/10/12</stime>
        <etime>2025/11/11</etime>
        <ongoing>false</ongoing>
        <amount>1000.0</amount>
        <frequency>5</frequency>
        <commission>5.0</commission>
        <percentage>
            <stock>
                NFLX
                <percent>35.0</percent>
            </stock>
            <stock>
                AAPL
                <percent>65.0</percent>
            </stock>
        </percentage>
    </plan>
</plans>
...
1. algs4_source_from_Procyon.jar -
    Used for plotting the performance chart of a portfolio over a given range.
    ...
    Setup for running applications as jar:
    While running the main program jar, we have included the above mentioned jar in the build,
    so no additional steps are needed.

    Setup in Intellij:
    ALong with the src, test and res folders, we have also included the libs folder in the zip.
    For setting up the jar file, right click on the libs after opening the entire project in
    Intellij. Click on - Add as Library.

GUI Steps:
1. If you are a new User, you can just click on "Create a new user",
...
You can choose to provide a pre-existing portfolio information file
by clicking on upload user portfolio.
Ensure that the file chosen is within the /users folder along with the program jar.

2. If you are an existing user, provide your username and password to login to the application.

The following are the options available for the user:
a. Examine a portfolio:
Select a portfolio from a list of portfolios associated with the user.
The stocks and its related information show up in the screen.

b. Compute the Value of Portfolio:
Select a portfolio from the list drop down and select a date on which you want to
see its performance.

c. Add New portfolio:
Add a new portfolio and its related stocks.
The parameters that are input are validated immediately by the program providing the opportunity
for the user to rectify any improper inputs.
(If the given user input is valid, the color of the text turns to green, if it is not of valid type
it turns to red)
Input all the stock details as per requirement. If the user wants to edit a stock after adding it,
they can click on the corresponding button that pops up in the below scroll pane.
Click on Submit to create a portfolio with all the stocks mentioned within it.

d. Add a new dollar cost averaging plan:
With this option the user gets to either apply a long term plan to his portfolio or create a new
portfolio and apply a long term cost averaging plan.
Specify the split of stock(in percentage) in the Stock details, also specify the start and end date
of the plan.
Create a plan by clicking on submit.

e. Buy stocks:
Buy new stocks into existing portfolio on a specific date.

f. Sell stocks:
Sell stocks on a specific date from existing portfolio.

g. Compute cost basis of a flexible portfolio:
Provides the total amount of investment made by the user by a given date.
THis feature also lets th user know about the amount he would be spending on future plans.
i.e future dates can be given as input.

h. Show performance of a portfolio:
The user can see the performance of one of his portfolios over a given date range.
A bar graph chart pops up with the bar graph information filled in it.
...
Template file "karthikjb10FIle.txt" containing a user with multiple portfolios with Port4 portfolio
having 3 different stocks. Please feel free to use it.

Step ii.
Pass just the name of the file from which input has to be read into the Text UI. i.e. karthikjb10FIle.txt
Please wait for sometime as we are pre-fetching all the stock values and loading it in LRU cache.
...
Step vi.
Input the day, month and year on which computation has to be made in the format dd-mm-yyyy
For example 14-10-2022
...
Enter the choice B to compute cost basis

Step xi.
Only the Flexible Portfolios show up in the list.
Enter Emergency Funds in this instance

Step xii.
Input the day, month and year on which the Compute Basis has to be calculated in the format dd-mm-yyyy
For example 14-10-2022

Step xiii.
The UI gives the total cost basis for the portfolio we wanted.
...
Enter the type of Portfolio you wish to create. Enter 1 or 2.
Input 1 for an Inflexible Portfolio
Input 2 for an Flexible Portfolio
...
Step vi.
Enter the date of purchase in dd-mm-yyy format.
For instance 10-02-2021

Step vii.
Enter the commission fees charged. Enter a value between $2 and $40.
For instance 3
...
Step xii.
Input the date on which computation has to be made in dd-mm-yyy format.
For example 30-03-2022
...
5.
The data of can be fetched on any day from 2000 January if the stock is in
existence and the value could be fetched by AlphaVantage API.